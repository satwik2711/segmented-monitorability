...
The chunker scores each reasoning block with the local `Qwen/Qwen3-0.6B-Base` model. Install its dependencies and run:

```sh
python3 -m pip install torch transformers numpy scipy
python3 src/core_chunker.py
```
...
```sh
python3 src/run_semantic_units.py --annotation-template data/mrt_boundary_annotations.jsonl
```
...
```sh
python3 src/run_monitorability.py --dry-run --output data/mrt_monitor_prompts.jsonl
```

For actual monitoring, use one fixed model at an OpenAI-compatible chat endpoint:

```sh
MONITOR_API_BASE=http://localhost:1234 python3 src/run_monitorability.py --model YOUR_MONITOR_MODEL
```
